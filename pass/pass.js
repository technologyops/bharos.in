...
etc.sha512Sync = async (...messages) => u8n(await subtle().digest('SHA-512', concatBytes(...messages).buffer));
...
const verifile = async (fname, msg) => {
  ...
  if (fname.endsWith('.pass')) {
    if (_istxt) {
      ...
      let passhdr = b32ToBytes(txtdecoder.decode(msg.slice(0,156)));
    ...
    if (fname.length < 157) { return false; }
    let passhdr
    try {
      passhdr = b32ToBytes(fname.slice(0,156));
    }
    catch (error) {
      return false;
    }
...
const signfile = async (msg, privKey, prefixpass=false) => {
  ...
  const publicKey = await getPublicKeyAsync(privKey);
  const hash = await etc.sha512Async(msg);
  const sig = await signAsync(hash, privKey);
  if (prefixpass) {
    if (istxt(msg)) {
      //b32pass+txt
      const txtdecoder = new TextDecoder('utf-8');
      return bytesToB32(Uint8Array.from([0, ...publicKey, ...sig])) + txtdecoder.decode(msg);
    } else {
      //binary
      return Uint8Array.from([0, ...publicKey, ...sig, ...msg]);
    }
  } else {
    return bytesToB32(Uint8Array.from([0, ...publicKey, ...sig]));
  }
};
